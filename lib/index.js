...
const FAMILIES = [
  ...
  { test: /imagine-image|image-quality|dall-e|flux|imagen|gpt-image/, input: ['text'], output: ['image'], efforts: false },
  // A catalog's explicit text-only declaration still takes precedence below.
  { test: /(?:^|[/:])gpt-5\.(?:[2-9]|[1-9]\d+)(?:[-.]|$)/, input: ['text', 'image'], output: ['text'], efforts: ['off', 'minimal', 'low', 'medium', 'high', 'xhigh'], format: 'openai' },
  { test: /(?:^|[/:])gpt-5\.1(?:[-.]|$)/, input: ['text', 'image'], output: ['text'], efforts: ['off', 'low', 'medium', 'high'], format: 'openai' },
  { test: /(?:^|[/:])gpt-5(?:[-.]|$)/, input: ['text', 'image'], output: ['text'], efforts: ['off', 'minimal', 'low', 'medium', 'high'], format: 'openai' },
  { test: /(?:^|[/:])gpt-(?:4\.1|4o)(?:[-.]|$)/, input: ['text', 'image'], output: ['text'], efforts: false },
...
async function readJsonBody(req) {
  ...
  for await (const chunk of req) {
    ...
    if (size > MAX_JSON_BODY_BYTES) {
      // Stop consuming an oversized body instead of leaving the socket half-read.
      req.destroy()
      return undefined
    }
    chunks.push(buffer)
...
async function workDir(ctx) {
  try {
    ...
    // A confined fs backend may refuse to resolve a path outside the session
    // workspace (the settings document lives under the DSH home). Probing only
    // needs some stable cwd, so report "unavailable" instead of failing the run.
    return undefined
  }
}
...
async function curlJson(ctx, cwd, method, url, apiKey, body) {
  ...
  return { ok: outcome.exitCode === 0 && status >= 200 && status < 300, status, body: bodyText }
}
  ...
  try {
    ...
    for (const id of Object.keys(current.routes)) {
      ...
      for (const model of route.models) {
        ...
        state.log.push(route.id + '/' + model.id + ' → ' + effortLabel(applied.reasoningEfforts) + ' · in ' + inputLabel(applied.input) + ' · out ' + detected.output.join(', ') + ' · ' + (api || 'openai-completions') + (completions && applied.compat && applied.compat.thinkingFormat ? '/' + applied.compat.thinkingFormat : ''))
      }
