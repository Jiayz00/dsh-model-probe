...
const FAMILIES = [
  ...
  { test: /deepseek-v4|deepseek-r1|reasoner/, input: ['text'], output: ['text'], efforts: ['off', 'low', 'high', 'max'], format: 'openai' },
  { test: /glm-5/, input: ['text'], output: ['text'], efforts: ['off', 'high', 'max'], format: 'openai' },
  { test: /qwen3\.8|qwen3-8/, input: ['text', 'image'], output: ['text'], efforts: ['off', 'low', 'medium', 'xhigh'], format: 'openai' },
  ...
  { test: /qwen/, input: ['text'], output: ['text'], efforts: ['off', 'low', 'medium', 'high'], format: 'openai' },
]
...
function applyDetection(model, detection) {
  ...
  try {
    ...
    for (const id of Object.keys(current.routes)) {
      ...
      for (const model of route.models) {
        ...
        const applied = applyDetection(model, payload)
        ...
        state.log.push(route.id + '/' + model.id + ' → ' + effortLabel(applied.reasoningEfforts) + ' · in ' + inputLabel(applied.input) + ' · out ' + detected.output.join(', '))
      }
