window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var zh = {
      ...
      intro: '先读网关 /models 返回的能力字段，再和内置的模型家族表合并取并集。按提供方接口适配：responses 写 reasoning.effort 映射，messages（completions）写 thinkingFormat 兼容开关。',
      protoResponses: 'responses 接口',
      protoMessages: 'messages 接口',
      heuristic: '按家族表写入',
      probe: '拉取目录后写入',
      overwrite: '覆盖已有配置',
    ...
    var en = {
      ...
      intro: 'Reads capability fields from the gateway /models list and merges them with the built-in family table (union). Adapts to the provider protocol: responses routes get a reasoning.effort map, messages (completions) routes get thinkingFormat compat switches.',
      protoResponses: 'responses API',
      protoMessages: 'messages API',
    ...
    function copyOf(t) {
      ...
      return {
        ...
        intro: t('intro'),
        protoResponses: t('protoResponses'),
        protoMessages: t('protoMessages'),
            ...
            : data.providers.map(function renderProvider(provider) {
              return React.createElement('section', { key: provider.id, className: 'mpb-group' },
                React.createElement('div', { className: 'mpb-groupName' },
                  ...
                  React.createElement('span', { className: 'mpb-proto', title: provider.api }, provider.api === 'openai-responses' ? text.protoResponses : text.protoMessages),
                ),
