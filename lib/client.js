window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      '.mpb-groupName{color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:1.5}',
    ...
    var zh = {
      ...
      intro: '先读网关 /models 返回的能力字段，再和内置的模型家族表合并。两边都有的档位都会保留。',
      heuristic: '按家族表写入',
      probe: '拉取目录后写入',
      overwrite: '覆盖已有配置',
    ...
    var en = {
      ...
      intro: 'Reads capability fields from the gateway /models list and merges them with the built-in family table. Levels present in either source are kept.',
          ...
          data.providers.length === 0
            ...
            : data.providers.map(function renderProvider(provider) {
              return React.createElement('section', { key: provider.id, className: 'mpb-group' },
                React.createElement('div', { className: 'mpb-groupName' }, provider.displayName || provider.id),
